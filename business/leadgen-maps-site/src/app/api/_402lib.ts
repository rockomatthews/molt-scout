...
export async function verifyUsdcPaymentTx(opts: {
  ...
  if (receipt.status !== "success") return { ok: false as const, reason: "tx_failed" as const };
  ...
  for (const log of receipt.logs) {
    ...
    try {
      ...
      return { ok: true as const, paidValue: args.value, paidTo: args.to, paidFrom: args.from };
    } catch {
      // ignore
    }
  }
